...
std::pair<double, double> RunVoronoiVisiumPipeline(const std::string& experiment_name,
                              const std::vector<std::vector<int>>& sparse_genes,
                              const std::vector<std::vector<double>>& spatial_coords,
                              int k,
                              double epsilon = 0.1) {
    std::cout << "--- Running " << experiment_name
              << " (N=" << spatial_coords.size() << ", k=" << k
              << ", eps=" << epsilon << ") ---\n";

    SpatialSketcher* my_sketcher =
        new VoronoiSketching(sparse_genes, spatial_coords, -1, epsilon);
    std::vector<std::size_t> final_sketch = my_sketcher->ComputeSketch(k);

    double rna_hausdorff = ComputeTranscriptomicHausdorff(sparse_genes, final_sketch);
    double spatial_hausdorff = ComputeCoordinateHausdorff(spatial_coords, final_sketch);

    std::cout << "Voronoi Stats\n";
    std::cout << "     [Metric] Transcriptomic Hausdorff (95%): " << rna_hausdorff << "\n";
    std::cout << "     [Metric] Coordinate Hausdorff (95%): " << spatial_hausdorff << "\n";

    ExportVisiumResultsToCSV("bin/" + experiment_name + "_results.csv",
                             spatial_coords, final_sketch);
...
int main() {
    ...
    for (int k : k_values) {
        ...
        std::future<std::pair<double, double>> f = std::async(RunBaselineVisiumPipeline, "baseline_k" + std::to_string(k), visium_genes, visium_coords, k);
        ...
        std::future<std::pair<double, double>> f2 = std::async(RunQuadtreeVisiumPipeline, "quadtree_k" + std::to_string(k), visium_genes, visium_coords, k, optimal_capacity, dynamic_budget);
        

        // Should define a RunVoronoiVisiumPipeline function that matches the signature of RunQuadtreeVisiumPipeline
        std::future<std::pair<double, double>> f3 = std::async(RunVoronoiVisiumPipeline, "voronoi_k" + std::to_string(k), visium_genes, visium_coords, k, .1);

        auto baseline_metrics = f.get();
        scaling_file << "Baseline," << k << "," << baseline_metrics.second << "," << baseline_metrics.first << "\n";

        auto quadtree_metrics = f2.get();
        scaling_file << "QuadTree," << k << "," << quadtree_metrics.second << "," << quadtree_metrics.first << "\n";

        auto voronoi_metrics = f3.get();
